...
class AzureTtsService(TtsService):
    ...
    async def tts(self, text: str, output_audio_filename: str, setting: Setting):
        ...
        """
        # Initializes the speech configuration using the Azure Speech SDK, obtaining the key and region from
        ...
        ssml = f'<speak version="1.0" xmlns="http://www.w3.org/2001/10/synthesis" xmlns:mstts="http://www.w3.org/2001/mstts" xml:lang="{setting.language}">' \
               f'<voice name="{setting.tts_voice_name}">' \
               f'{text}</voice></speak>' 
        #f'<lexicon uri="https://raw.githubusercontent.com/lionlu/azure/main/lexicon.xml"/>' \
        #ssml = ssml.replace("4214", '<phoneme alphabet="sapi" ph="si 4 - er 4 - yi 1 - si 4">4214</phoneme>')
        ssml = ssml.replace("Shen Zile", "沈智乐")
        ssml = ssml.replace("Xi’an", "西安")
        ssml = ssml.replace("roujiamo", "肉夹馍")
        self.logger.info(f"SSML: {ssml}")
